...
\def\tempYes{yes}
\def\tempNo{no}
\documentclass[10pt,sigconf,letterpaper,dvipsnames\ifx\removeHeaders\tempYes ,nonacm\fi\ifx\removeHeaders\tempNo ,anonymous\fi]{acmart}
\acmYear{2019}
\copyrightyear{2019}
...
\caption{Illustration of the interval mechanism used by our \gls{qdisc}. The multiplier used is $\frac{1}{2}$. Each \textit{guard interval} (GI) contains one or more intervals of which the longest one is considered the \textit{longest interval} (LI). The \gls{gi} is always at least the length of the previous \gls{li} times the multiplier. A \gls{gi} ends at the first packet loss after that is after the previous \gls{li} times the multiplier.}
\label{fig:intervals}
\end{figure}
...
We evaluate our \gls{qdisc} using the \ifx\removeHeaders\tempYes{}\texttt{py-virtnet}\else{}[removed for anonymity]\fi{} toolkit to build a virtual network using Linux's network namespaces. We initialize the buffer size for our \gls{qdisc} to 100 packets per flow like in fq. 
...
\caption{Throughput (\ref{fig:fqThroughput}) and delay (\ref{fig:fqDelay}) of a Cubic flow that is managed by fq at the bottleneck. The initial bandwidth is 20\,Mbit/s but it is halved after 30\,s. The delay is 10\,ms. It is clear that there is a standing queue and after the bandwidth halves, the minimum delay is 100\,ms even though 10\,ms would be the possible.}
\label{fig:fq}
\end{figure}
...
\caption{Throughput (\ref{fig:cnThroughput}) and delay (\ref{fig:cnDelay}) of a Cubic flow that is managed by fq at the bottleneck. The initial bandwidth is 20\,Mbit/s but it is halved after 30\,s. The delay is 10\,ms. Our \gls{qdisc} manages to keep the queuing delay minimal while achieving the same throughput as with an oversized buffer.}
\label{fig:cn}
\end{figure}
...
\caption{Throughput (\ref{fig:cnThroughput2}) and delay (\ref{fig:cnDelay2}) of a Cubic flow that is managed by fq at the bottleneck. The initial bandwidth is 20\,Mbit/s but it is doubled after 30\,s. The delay is 10\,ms. Also a sudden large increase of bandwidth is handled well by our \gls{qdisc}.}
\label{fig:cn2}
\end{figure}
...
\begin{figure}[h]
\subfloat[Throughput of our \gls{qdisc}\label{fig:fqCodelThroughputLong}
]{\includegraphics[width=\columnwidth]{{{../plots/throughput_1_fq_codel_cubic_100_100_240_1.0_bw_1571244312838.pcap}}}}{}
\subfloat[Delay of our \gls{qdisc}.\label{fig:fqCodelDelayLong}
]{\includegraphics[width=\columnwidth]{{{../plots/rtt_1_fq_codel_cubic_100_100_240_1.0_bw_1571244312838.pcap}}}}{}
\caption{Throughput (\ref{fig:fqCodelThroughputLong}) and delay (\ref{fig:fqCodelDelayLong}) of a Cubic flow that is managed by fq\_codel at the bottleneck. The bandwidth is 100\,Mbit/s. The delay is 100\,ms. The average \gls{rtt} is 101.36\,ms. Total throughput is 2615\,MB.}
\label{fig:fqCodel}
\end{figure}
...
\caption{Throughput (\ref{fig:cnThroughputLong}) and delay (\ref{fig:cnDelayLong}) of a Cubic flow that is managed by our \gls{qdisc} at the bottleneck. The bandwidth is 100\,Mbit/s. The delay is 100\,ms. The average \gls{rtt} is 129.45\,ms. Total throughput is 2893\,MB.}
\label{fig:cnLong}
\end{figure}

Next we compare our \gls{qdisc} against fq\_codel. Figure\autoref{fig:fqCodelDelayLong} shows that fq\_codel keeps the queuing delay under 5\,ms. Figure\autoref{fig:fqCodelThroughputLong} shows that keeping the queuing delay that low is detrimental for achieving full throughput when using the Cubic \gls{cca}. With our \gls{qdisc} we achieve throughput that is more than 10\% higher overall, while only slightly increasing the average delay \todo{More systematic evaluation}. In addition to Cubic, we performed experiments with Reno. Here, over a 240\,s flow of 100\,Mbit/s with a delay of 50\,ms, our \gls{qdisc} achieves more than 20\% higher throughput than fq\_codel. The average \gls{rtt} for fq\_codel is 51.13\,ms while it is 96.59\,ms for our \gls{qdisc}.
